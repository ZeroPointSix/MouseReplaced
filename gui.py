...
class KeyMouseGUI:
    ...
    def create_settings_ui(self):
        ...
        self.caps_mult_var = tk.DoubleVar(
            value=self.config_parser.getfloat(
                'Settings',
                'mouse_speed_capslock_multiplier'
            )
        )
        ttk.Scale(
            speed_frame,
    ...
    def reset_defaults(self):
        ...
        if messagebox.askyesno("确认", "确定要恢复所有设置到出厂默认值吗？"):
            ...
            self.shift_mult_var.set(
                setting_defaults['mouse_speed_shift_multiplier']
            )
            self.caps_mult_var.set(
                setting_defaults['mouse_speed_capslock_multiplier']
            )
