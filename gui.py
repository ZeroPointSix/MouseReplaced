...
class FormattedLabel(ttk.Label):
    ...
    def __init__(self, parent, textvariable, precision=1, **kwargs):
        ...
        self._update_text()  # 初始化显示

    def _update_text(self, *args):
        """当绑定的DoubleVar变化时，更新显示的文本"""
        try:
            value = self.var.get()
            formatted_text = f"{value:.{self.precision}f}"
            self.str_var.set(formatted_text)
...
class KeyMouseGUI:
    def __init__(self, root, tray_icon=None):
        ...
        ttk.Button(self.button_frame, text="保存设置", 
                  command=self.save_settings).pack(side=tk.RIGHT, padx=5)
        ttk.Button(self.button_frame, text="保存为默认",
                  command=self.save_current_as_default).pack(side=tk.RIGHT, padx=5)
        ttk.Button(self.button_frame, text="恢复默认",
                  command=self.reset_defaults).pack(side=tk.RIGHT, padx=5)
    ...
    def create_key_setting(self, parent, label_text, config_key, row):
        ...
        ttk.Button(parent, text="设置", command=command).grid(
    ...
    def create_settings_ui(self):
        ...
        ttk.Scale(speed_frame, from_=1, to=50,
        ...
        self.shift_mult_var = tk.DoubleVar(
            value=self.config_parser.getfloat(
                'Settings', 'mouse_speed_shift_multiplier'))
        ttk.Scale(speed_frame, from_=0.1, to=2.0,
                 variable=self.shift_mult_var,
                 orient=tk.HORIZONTAL).grid(
                     row=1, column=1, padx=5, pady=5, sticky=tk.EW)
        FormattedLabel(speed_frame, textvariable=self.shift_mult_var).grid(
        ...
        self.caps_mult_var = tk.DoubleVar(
            value=self.config_parser.getfloat(
                'Settings', 'mouse_speed_capslock_multiplier'))
        ttk.Scale(speed_frame, from_=0.1, to=2.0,
                 variable=self.caps_mult_var,
                 orient=tk.HORIZONTAL).grid(
                     row=2, column=1, padx=5, pady=5, sticky=tk.EW)
        FormattedLabel(speed_frame, textvariable=self.caps_mult_var).grid(
        ...
        ttk.Scale(smooth_scroll_frame, from_=50, to=500,
                 variable=self.initial_velocity_var,
                 orient=tk.HORIZONTAL).grid(
                     row=0, column=1, padx=5, pady=5, sticky=tk.EW)
        FormattedLabel(smooth_scroll_frame,
                      textvariable=self.initial_velocity_var).grid(
        ...
        ttk.Scale(smooth_scroll_frame, from_=500, to=6000,
                 variable=self.max_velocity_var,
                 orient=tk.HORIZONTAL).grid(
                     row=1, column=1, padx=5, pady=5, sticky=tk.EW)
        FormattedLabel(smooth_scroll_frame,
                      textvariable=self.max_velocity_var).grid(
                          row=1, column=2, padx=5, pady=5)
        ...
        ttk.Scale(smooth_scroll_frame, from_=100, to=2000,
                 variable=self.acceleration_var,
                 orient=tk.HORIZONTAL).grid(
                     row=2, column=1, padx=5, pady=5, sticky=tk.EW)
        FormattedLabel(smooth_scroll_frame,
                      textvariable=self.acceleration_var).grid(
                          row=2, column=2, padx=5, pady=5)
        ...
        ttk.Scale(perf_frame, from_=0.001, to=0.05,
                 variable=self.delay_var,
                 orient=tk.HORIZONTAL).grid(
                     row=0, column=1, padx=5, pady=5, sticky=tk.EW)
        FormattedLabel(perf_frame, textvariable=self.delay_var,
                      precision=3).grid(row=0, column=2, padx=5, pady=5)
