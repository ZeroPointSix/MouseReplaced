...
class MouseActionManager:
    ...
    def handle_left_button_event(self, is_key_down: bool):
        if is_key_down:
            if not self.mouse_state.is_left_mouse_button_held_by_keyboard: self.mouse_controller.press(pynput.mouse.Button.left); self.mouse_state.is_left_mouse_button_held_by_keyboard = True
        else:
            if self.mouse_state.is_left_mouse_button_held_by_keyboard: self.mouse_controller.release(pynput.mouse.Button.left); self.mouse_state.is_left_mouse_button_held_by_keyboard = False
    def handle_right_button_event(self, is_key_down: bool):
        if is_key_down:
            if not self.mouse_state.is_right_mouse_button_held_by_keyboard: self.mouse_controller.press(pynput.mouse.Button.right); self.mouse_state.is_right_mouse_button_held_by_keyboard = True
        else:
            if self.mouse_state.is_right_mouse_button_held_by_keyboard: self.mouse_controller.release(pynput.mouse.Button.right); self.mouse_state.is_right_mouse_button_held_by_keyboard = False
    def handle_middle_button_event(self, is_key_down: bool):
        if is_key_down:
            if not self.mouse_state.is_middle_mouse_button_held_by_keyboard: self.mouse_controller.press(pynput.mouse.Button.middle); self.mouse_state.is_middle_mouse_button_held_by_keyboard = True
        else:
            if self.mouse_state.is_middle_mouse_button_held_by_keyboard: self.mouse_controller.release(pynput.mouse.Button.middle); self.mouse_state.is_middle_mouse_button_held_by_keyboard = False

    def start_scrolling_down(self): self.scroll_controller.start_scroll_down()
    def stop_scrolling_down(self): self.scroll_controller.stop_scroll_down()
    def start_scrolling_up(self): self.scroll_controller.start_scroll_up()
    def stop_scrolling_up(self): self.scroll_controller.stop_scroll_up()
    ...
    def mouse_movement_worker(self, stop_event):
        ...
        while not stop_event.is_set():
            ...
            if self.mode_switch.mouse_control_mode_active:
                ...
                if self.mouse_state.mouse_speed_shift_active: speed_multiplier *= cfg.MOUSE_SPEED_SHIFT
                if self.mouse_state.mouse_speed_caplock_active: speed_multiplier *= cfg.MOUSE_SPEED_CAPLOCK
...
class MouseControl:
    ...
    def on_press(self, key):
        if key in (pynput.keyboard.Key.alt_l, pynput.keyboard.Key.alt_r): self.hotkey_modifiers.add('alt')
        try:
            if key.char.lower() == self.HOTKEY_TRIGGER_KEY and 'alt' in self.hotkey_modifiers: self.mode_switch.on_alt_a_activated()
        ...
        if key == self.config.EXIT_PROGRAM_PYNPUT and self.mode_switch.mouse_control_mode_active:
            if self.mode_switch.tray_icon: self.mode_switch.tray_icon.on_exit()
            return False
            
    def on_release(self, key):
        if key in (pynput.keyboard.Key.alt_l, pynput.keyboard.Key.alt_r): self.hotkey_modifiers.discard('alt')
        if key == pynput.keyboard.Key.shift_l: self.mouse_state.mouse_speed_shift_active = False
    ...
    def win32_event_filter(self, msg, data):
        ...
        if data.vkCode in self.config.MOUSE_CONTROL_VKS and self.mode_switch.mouse_control_mode_active:
            ...
            if vk == cfg.LEFT_CLICK_VK: self.mouse_action.handle_left_button_event(is_key_down)
                ...
                if is_key_down: self.mouse_action.start_scrolling_down()
                else: self.mouse_action.stop_scrolling_down()
            
            elif vk == cfg.SCROLL_UP_VK:
                if is_key_down: self.mouse_action.start_scrolling_up()
                else: self.mouse_action.stop_scrolling_up()

            elif vk in {cfg.MOVE_UP_VK, cfg.MOVE_DOWN_VK, cfg.MOVE_LEFT_VK, cfg.MOVE_RIGHT_VK}:
                key_map = {
